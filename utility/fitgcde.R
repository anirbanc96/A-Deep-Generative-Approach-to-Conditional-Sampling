...
)
{
    ...
    if(!is.matrix(train.x) | !is.matrix(train.y) |
       !is.matrix(test.x)  | !is.matrix(test.y) ) stop("The dataset are not matrix!")
    dim.x <- ncol(train.x)
    dim.y <- ncol(train.y)
    ...
    discriminator_input_x <- layer_input(shape = c(dim.x), dtype = "float32")
    discriminator_input_y <- layer_input(shape = c(dim.y), dtype = "float32")
    ...
    gene.input.x              <- layer_input(shape = c(dim.x), dtype = "float32")
    gene.input.eta            <- layer_input(shape = c(dim.eta), dtype = "float32")
        ...
        for (step in stepfrom:iterations) {
            ...
            if(disc.train.type == "batch"){
                while (train.disc.count <= step.disc) {
                    ...
                    sgd.x.disc <- array(sgd.x.disc, dim = dim(sgd.x.disc))
                    combined <- array(combined, dim = dim(combined))
                    v.labels <- array(v.labels)
